...
自动学习只采样短窗口、小范围替换：语音输出后 30 秒内，用户用 Backspace/Delete 删除刚输出文本的一小段并继续手动输入新内容，遇到 Enter、下一次语音输入、超时、鼠标点击或窗口失焦时结束评估。整段大改写、删除后未输入、纯标点、过短片段和明显无关内容不会生成规则。
...
桌面端新增“个人词库”管理页，可以搜索、手动新增、启用/禁用和删除纠正记忆；这些操作只改本机文件。对于本地 `glm_asr_2512` provider，engine 会把已启用 target 合并为最多 100 个 hotwords；`typeup_backend` provider 暂时只做本地后处理，因为后端 `/v1/stt/transcribe` 当前不接收 hotwords。AI 微润色和编辑提示也会带上近期高置信个人词汇，提醒模型优先保留这些写法。
