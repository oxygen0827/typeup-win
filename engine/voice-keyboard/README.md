...
- [x] 微润色预览模式：语音先显示在状态框预览卡，分句轻润色，结束后整体润色，并在用户确认后才输出
...
双击 `ptt_key` 可在原生输入和微润色模式之间切换。微润色模式下不会立即写入目标输入框：录音内容会先显示在桌面状态框上方的白色预览卡里，停顿时逐句做保守润色，松开热键后再做整体微润色，并弹出确认窗口。只有用户确认后，最终文本才会输入到当前光标位置。
