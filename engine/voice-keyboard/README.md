...
- [x] 微润色预览模式：语音先显示在状态栏上方独立同宽预览卡，分句轻润色，结束后整体润色，并在用户确认后才输出
- [x] STT 接入（讯飞 xunfei / 阿里云 NLS / 火山引擎 / 智谱 GLM-4-Voice / OpenAI Whisper）
- [x] AI 键（ai_key）：Instruction Mode，STT + 意图分类 + Voice Text Operation，自动执行以下操作：
  - 快捷键：说出操作名称直接触发系统快捷键
  - 编辑：修改/润色 Explicit Selection 或最近一次 TypeUp 输出的 Tracked Segment
  - 写作：完整生成内容后一次性插入，并做中文标点兜底
  - 删除：删除全文或选中内容；无选区局部删除会提示先选中
  - 撤销：触发当前应用的系统撤销快捷键
  - 聊天：AI 回复显示在状态框/HUD，不写入目标输入框
...
双击 `ptt_key` 可在原生输入和微润色模式之间切换。微润色模式下不会立即写入目标输入框：录音内容会先显示在桌面状态栏上方的白色预览卡里。预览卡与下方状态栏同宽、视觉上独立成两个圆角小框，并会随文本行数自动增高。停顿时逐句做保守润色，松开热键后再做整体微润色，并弹出确认窗口。只有用户确认后，最终文本才会输入到当前光标位置。
...
明确要求修改已有文字，AI 会修改选中内容；没有选区时默认修改最近一次由 TypeUp 输出的内容：

| 你说的 | 效果 |
|--------|------|
| "把会议改成会谈" | 在操作窗口内替换关键词 |
| "去掉最后一句" | 删除明确匹配到的目标文本 |
| "改成更正式的表达" | 生成 Replacement Plan，经本地校验后替换 |
| "帮我翻译成英文" | 翻译选中内容或最近一次 TypeUp 输出 |

> 用鼠标选中文字后再按 AI 键，优先修改选中内容。Windows 端暂不读取任意应用的光标周边文本；无选区且没有最近一次 TypeUp 输出时，会提示先选中内容。

#### 3. 写作

给出主题或要求，AI 完整生成后一次性插入输入框：

| 你说的 | 效果 |
|--------|------|
| "帮我写一段单片机的介绍" | 生成介绍文字并插入 |
| "写一封请假邮件" | 生成邮件正文 |
| "给这个功能写一段产品描述" | 生成描述文字 |

#### 4. 删除

明确要求删除时，AI 会删除全文或选中内容；局部删除必须先选中目标：

| 你说的 | 效果 |
|--------|------|
| "删除这段话" | 未选中时会提示先选中内容 |
| "删除全文" | 清空当前可操作窗口，必要时走全选删除兜底 |
| "清除选中内容" | 删掉鼠标选中的文字 |
...
其他问题或不确定意图，AI 只在状态框/HUD 显示回复，不写入目标输入框：

| 你说的 | 效果 |
|--------|------|
| "你能做什么" | AI 介绍本软件功能 |
| "这句话有没有语法错误" | AI 在状态框回复 |
| "帮我想个标题" | AI 在状态框给出建议 |
