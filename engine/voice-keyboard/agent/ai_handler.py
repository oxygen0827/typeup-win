...
def _parse_intent_result(raw: str) -> dict:
    """Compatibility helper used by older Windows-side tests and diagnostics."""
...
def _fallback_intent_after_classification_error(text: str, selected: str, context: str) -> dict:
    """Compatibility fallback for callers that still use the old helper."""
    if looks_like_edit_instruction(text) and (selected or context):
        return {"type": "edit"}
